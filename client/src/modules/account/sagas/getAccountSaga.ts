...
import {SuccessAsyncProperty, AccountDto, TokenDto} from 'template-common';
...
export default function* getAccountSaga(action: ActionType<typeof getAccountAction.request>) {
  try {
    ...
    const accountResponse: AxiosResponse<AccountDto> = yield call(getAccountEndPoint, null, token.value);
